...
export default class GrupoUsuariosController {
    ...
    public async setRegistrarUsuarioGrupo({request, response}: HttpContextContract) {
        try {
            ...
            const datosExistentes: Number = await this.getValidarDatosGrupoUsuario(codigoGrupo, codigoUsuario)
    ...
    private async getValidarDatosGrupoUsuario(codigo_grupo: Number, codigo_usuario: Number): Promise<Number> {
